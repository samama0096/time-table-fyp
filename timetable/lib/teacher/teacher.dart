...
class TeacherViewTimetable extends StatefulWidget {
  const TeacherViewTimetable({super.key, required this.name});
  final String name;
  @override
...
class _TeacherViewTimetableState extends State<TeacherViewTimetable> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder(
          future: FirebasService.getTeacherWiseData(widget.name),
              ...
              return Center(
                child: ListView.builder(
                    ...
                    itemBuilder: (context, i) {
                      TeacherByDay dayData = snap.data![i];
                      return Padding(
...
String? getString(int slot, TeacherByDay tt) {
  if (slot == 1) {
    return tt.s0830;
  }
  if (slot == 2) {
